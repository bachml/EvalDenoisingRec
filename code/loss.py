...
def drop_rate_schedule(iteration, param):
    drop_rate = np.linspace(0, param["drop_rate"]**param["exponent"], param["num_gradual"])
    if iteration < param["num_gradual"]:
        return drop_rate[iteration]
    else:
        return param["drop_rate"]



def loss_function_TCE(y, t, drop_rate):
...
def loss_function_TCE_without_reduction(y, t, drop_rate):
    loss = F.binary_cross_entropy_with_logits(y, t,  reduction='none')
    ...
    loss_update = F.binary_cross_entropy_with_logits(y[ind_update], t[ind_update],  reduction='none')

    return loss_update


def loss_function_RCE(y, t, alpha):
    loss = F.binary_cross_entropy_with_logits(y, t,  reduction='none')
...
def loss_function_RCE_without_reduction(y, t, alpha, reduction='none'):
